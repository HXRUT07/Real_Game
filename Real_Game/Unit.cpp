...
void Unit::draw(sf::RenderWindow& window, int stackCount)
{
    ...
    if (m_owner == 1)
    {
        tex = (stackCount >= 2) ? &s_texArmy : &s_texKnight;
    }
    else
    {
        tex = (stackCount >= 2) ? &s_texGoblinArmy : &s_texGoblin;
    }
    ...
    if (texSize.x > 0 && texSize.y > 0)
    {
        ...
        // ใช้ความกว้างเป็นหลัก crop เป็นสี่เหลี่ยมจตุรัส
        int cropSize = (int)std::min((float)texSize.x, (float)texSize.y);

        // center X
        int offsetX = ((int)texSize.x - cropSize) / 2;

        // เปรมทำ - เลื่อน Y ขึ้นด้านบน 15% เพื่อให้เห็นหน้าชัด
        int offsetY = (int)(((int)texSize.y - cropSize) * 0.10f);
        // เปรมทำ - จบ
    ...
    if (m_owner == 1)
    {
        ...
        m_circle.setOutlineColor(m_currentAP > 0
            ? sf::Color(255, 215, 0)   // ทอง = มี AP
            : sf::Color(120, 120, 120)); // เทา = หมด AP
    }
    else
    {
        m_circle.setFillColor(m_currentAP > 0
